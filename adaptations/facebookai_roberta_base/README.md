# FacebookAI/roberta-base Ascend NPU Adaptation

## 模型信息

- **Model ID**: [FacebookAI/roberta-base](https://huggingface.co/FacebookAI/roberta-base)
- **架构**: roberta (RobertaForMaskedLM)
- **任务**: 完形填空 / 掩码语言模型 (fill-mask)，非生成式 encoder
- **语言**: en
- **规模**: ~125M 参数, 12 层, hidden 768, 12 heads, float32

## 验证方式

非生成式模型，不使用 `generate()`；demo 通过**前向 logits + `<mask>` 位置 argmax 填空**验证：

- 输入: `The capital of France is <mask>.`（RoBERTa 的 mask token 是 `<mask>`）
...
仅验证架构与代码路径，不下载权重；层数保守缩小到 2 以加速初始化（`num_hidden_layers: 12 -> 2`）。
...
加载预训练权重（float32, ~500MB），模型与 tokenizer 缓存到本目录 `models/`。
...
- transformers 原生支持架构（RobertaForMaskedLM），无自定义代码。
- tokenizer 为 GPT-2 风格 BPE（vocab.json + merges.txt + tokenizer.json），无需 sentencepiece/tiktoken。
