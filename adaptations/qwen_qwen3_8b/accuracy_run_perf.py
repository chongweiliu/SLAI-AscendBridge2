#!/usr/bin/env python3
"""
accuracy_run_perf.py for Qwen/Qwen3-8B — NPU 性能优化版（runtime_only）。
...
def load_benchmark_texts() -> tuple[list[str], str]:
    ...
    if wikitext_jsonl is not None:
        print(f"[perf] loading dataset from {wikitext_jsonl}")
        texts = []
        ...
        print(f"[perf] loaded {len(texts)} samples from wikitext (jsonl)")
        return texts, "wikitext"
...
def batched_teacher_forcing(model, tokenizer, texts, device, batch_size, max_length=MAX_LENGTH):
    """批量 teacher-forcing 前向推理，提取每条样本最后一个 token 的 logits 和 perplexity。"""
    ...
    with torch.no_grad():
        for batch_start in range(0, len(texts), batch_size):
            ...
            for j in range(len(batch_texts)):
                ...
                sample_ids = input_ids[j, :seq_len_j].unsqueeze(0)
                sample_logits = logits[j, :seq_len_j, :].unsqueeze(0)
...
def cmd_run(args):
    ...
    warmup_encoded = tokenizer(warmup_texts, return_tensors="pt", truncation=True, max_length=MAX_LENGTH, padding=True).to(first_device)
    with torch.no_grad():
...
def cmd_compare(args):
    ...
    optimizations_str = "runtime_only: warmup(3x) + TASK_QUEUE_ENABLE=1 + batched_teacher_forcing(bs=1)"

    result_entry = {
        "dtype": b_metric.get("dtype", "bf16"),
        "mode": b_metric.get("mode", "pretrained"),
        "dataset": b_metric.get("dataset", "wikitext"),
        "output_type": "logits",
        ...
        "optimization_items": ["warmup_3x", "TASK_QUEUE_ENABLE", "batched_teacher_forcing_bs1"],
        "optimization_kind": "runtime_only",
        "selected_npu": selected_npu,
...
def main():
    parser = argparse.ArgumentParser(description="accuracy_run_perf.py for Qwen/Qwen3-8B (runtime_only)")
