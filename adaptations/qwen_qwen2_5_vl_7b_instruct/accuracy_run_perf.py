#!/usr/bin/env python3
"""
NPU Optimized accuracy_run_perf.py for Qwen/Qwen2.5-VL-7B-Instruct.
Runtime-only optimization: warmup(3x) + TASK_QUEUE_ENABLE=1 + batched_inference(bs=2, right-padding).

Optimization strategy: runtime_only (no model code modification)
  - warmup 3 forward passes to prime NPU operator compilation cache
  - TASK_QUEUE_ENABLE=1 async operator dispatch, reduces Host-Device sync
  - Symmetric warmup: baseline and perf both use same warmup strategy
  - Batched inference (bs=2) with right-padding for better NPU utilization

Usage:
    # Run perf (set TASK_QUEUE_ENABLE=1 externally)
...
Model Type: qwen2_5_vl (VLM, text-only teacher-forcing path)
Dataset: wikitext
"""
...
# Dataset config (same as accuracy_run.py)
...
def get_device(force_cpu: bool = False):
    """Get inference device. NPU card selection: mem_get_info picks card with most free HBM."""
    if force_cpu:
        return "cpu", 0, "cpu"
    try:
...
def setup_model(use_pretrained: bool, device, cache_dir: str):
    """Load model (VLM text-only path). Single-device loading, no device_map='auto'."""
    ...
    if use_pretrained:
        # Tier2: load pretrained weights; failure raises directly (no silent fallback to config)
        model = AutoModelCls.from_pretrained(
            MODEL_ID, trust_remote_code=True, torch_dtype="auto", cache_dir=cache_dir
        )
        model = model.to(device)
    else:
        # Tier1: random weights
...
def cmd_run(args):
    ...
    # Right-padding keeps real tokens at the same positions as single-sample,
    # reducing bf16 numerical differences
    ...
    # For non-generation tasks, latency_s = wall_clock_s / num_samples
...
def cmd_compare(args):
    ...
    # Find baseline and perf metrics (pretrained mode)
    import glob
    ...
    dtype_str = baseline_metrics.get("dtype", "bf16")
    ...
    # For non-generation tasks, latency_s must equal wall_clock_s / num_samples
    ...
    result_entry = {
        ...
        "validation_note": "Independent baseline artifact comparison, not self-baseline; symmetric warmup(3x) steady_state measurement; not cold-start vs warm-start.",
...
def main():
    parser = argparse.ArgumentParser(description="NPU optimized accuracy run for Qwen2.5-VL-7B-Instruct")
