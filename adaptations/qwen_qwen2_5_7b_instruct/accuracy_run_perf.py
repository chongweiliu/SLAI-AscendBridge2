"""
NPU Optimized accuracy_run_perf.py for Qwen/Qwen2.5-7B-Instruct.
Runtime-only optimization: warmup(3x) + TASK_QUEUE_ENABLE=1.
...
def setup_model(use_pretrained: bool, device, cache_dir: str):
    """加载模型 (CausalLM)。单卡加载，禁用 device_map="auto"。"""
...
def cmd_run(args):
    """Run perf: warmup + 50 sample teacher-forcing inference."""
    SEED = 42
    random.seed(SEED)
    np.random.seed(SEED)
...
def main():
    parser = argparse.ArgumentParser(description="NPU optimized accuracy run for Qwen2.5-7B-Instruct")
