...
def run_step2(model, tokenizer, first_device, device_short, mode_str, adapt_dir: Path, dataset_name: str, texts: list[str], max_samples: int = 250):
    ...
    all_ttft = []
    all_tpot = []
    all_sample_latency = []  # 每样本端到端延迟（forward + 生成）
    ...
    return outputs_path, ttft_avg, tpot_avg, avg_latency_s
...
def main():
    ...
    outputs_path, ttft_avg, tpot_avg, avg_latency_s = run_step2(model, processor, first_device, device_short, mode_str, ADAPT_DIR, dataset_name, texts, args.max_samples)
