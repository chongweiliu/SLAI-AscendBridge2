"""
NPU Optimized accuracy_run_perf.py for Qwen/Qwen3.5-9B (multimodal qwen3_5, text-only path).
Runtime-only optimization: warmup(3x) + TASK_QUEUE_ENABLE=1 + batched_inference(bs=2).

优化策略: runtime_only (无模型代码修改)
  - warmup 3 次前向推理，预热 NPU 算子编译缓存
  - TASK_QUEUE_ENABLE=1 异步算子下发，减少 Host-Device 同步等待
  - batched teacher-forcing (bs=2, right-padding) 提升 NPU 利用率
...
def setup_model(use_pretrained: bool, device, cache_dir: str):
    """加载模型 (Qwen3.5-9B 多模态 qwen3_5 架构, AutoModelForImageTextToText)。
    单卡加载，禁用 device_map="auto"。"""
    ...
    if use_pretrained:
        # Tier2: 加载预训练权重；失败直接抛错（禁止 silent fallback 到 config）
...
def cmd_run(args):
    ...
    try:
        if device_short == "npu" and first_device.index is not None:
            peak_mem = torch.npu.max_memory_allocated(first_device.index) / (1024**2)
        elif device_short == "cuda" and first_device.index is not None:
            peak_mem = torch.cuda.max_memory_allocated(first_device.index) / (1024**2)
    except Exception:
        pass
    ...
    print(f"[perf] batch_size: {batch_size}")
...
def cmd_compare(args):
    ...
    if baseline_memory_mb and perf_memory_mb and baseline_memory_mb > 0:
        memory_reduction_pct = round((1 - perf_memory_mb / baseline_memory_mb) * 100, 2)
...
def main():
    parser = argparse.ArgumentParser(description="NPU optimized accuracy run for Qwen3.5-9B")
