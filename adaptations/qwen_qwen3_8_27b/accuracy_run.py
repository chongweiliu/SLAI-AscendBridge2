#!/usr/bin/env python3
"""Benchmark for Qwen/Qwen3.8-27B (multimodal qwen3_5, text-only generation path).
...
def main():
    ...
    if args.use_pretrained:
        print("[Setup] Loading pretrained weights (fp16, single card)...")
        model = AutoModelCls.from_pretrained(MODEL_ID, trust_remote_code=True, torch_dtype=torch.float16, cache_dir=cache_dir)
        model = model.to(device)
    else:
        print("[Setup] DRY/config mode: random weights (full text backbone)")
        model = AutoModelCls.from_config(config, trust_remote_code=True, torch_dtype=torch.float16)
        model = model.to(device)
    model.eval()
