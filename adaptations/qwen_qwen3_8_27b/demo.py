"""
Ascend NPU adaptation demo for Qwen/Qwen3.8-27B (multimodal, qwen3_5, 27B).

架构: Qwen3_5ForConditionalGeneration — 视觉编码器 + 混合注意力文本主干
(64 层: 3/4 linear_attention + 1/4 full_attention, mrope)。
...
大模型说明: 27B bf16 权重约 54GB，单卡 64GB HBM 紧张，Full Run 用
device_map="auto" 跨 2 卡分片；若单环境只有 1 卡则自动退化为单卡加载。
"""
...
# qwen3_5 多模态主类：AutoModelForImageTextToText（旧版本回退 AutoModelForVision2Seq）
try:
...
def get_device(device_index: int = 0):
    ...
    try:
        ...
        if hasattr(torch, "npu") and torch.npu.is_available():
            ...
            # 多卡共享环境：set_device 选主卡（不用 ASCEND_RT_VISIBLE_DEVICES）
...
# ============================================================
def main():
    parser = argparse.ArgumentParser(description="Ascend Adaptation Demo: Qwen/Qwen3.8-27B")
    ...
    device, dev_count = get_device(args.device_index)
    assert device.startswith(("npu", "cuda")), f"Need NPU or CUDA, got device={device}"
    print(f"[Setup] Using device: {device} (accelerator count: {dev_count})")
    ...
    # ============================================================
    # --dry-run: 随机权重 + 缩小层数（64 -> 4），快速验证架构与代码路径，不下载权重。
    # 无 --dry-run: 加载预训练权重（27B bf16 约 54GB），device_map="auto" 跨卡分片。
    ...
    if args.dry_run:
        ...
        print("[Setup] Attempting to load pre-trained weights (~54GB bf16)...")
