...
> **Deployer 选择规则**：本节仅作为用户显式要求 Ray 时的参考。
> 自动部署默认并优先使用 vLLM 原生 `mp`；不得因为本节、模型教程、
> 已安装 Ray 或已有 Ray 集群而自动选择 Ray，也不得在 MP 失败后自动回退。
...
**何时使用**：模型单机装不下、需要跨机 NPU 资源，且用户在当前提示词中明确要求 Ray 时。
