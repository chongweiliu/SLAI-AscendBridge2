> 来源：vllm-ascend docs/source/tutorials/models/Kimi-K3.md（releases/v0.23.0 分支，首次支持于 vLLM-Ascend 0.23.0，抓取于 2026-07-28）

# Kimi-K3 部署知识库

## 关键信息速览（中文提炼）

- **架构类型**：原生多模态 MoE。语言骨干 = Kimi Delta Attention (KDA) + 周期性 Gated Multi-head Latent Attention (MLA)；专家计算 = Stable LatentMoE；集成 MoonViT 视觉编码器。支持文本/图像理解、推理、工具调用（tool calling）。
- **首次支持版本**：vLLM-Ascend 0.23.0（Kimi-K3 在此版本首次支持）。
- **硬件要求**：Atlas 800 A3（64G × 16），至少 4 节点。
- **量化**：ModelSlim W4A8；权重 `Eco-Tech/Kimi-K3-w4a8`（ModelScope）。本地实现走 `--quantization ascend`；若 checkpoint 已含 `compressed-tensors` 量化配置，则省略 `--quantization ascend`，让 vLLM 从 checkpoint 自动发现。
- **部署拓扑**：4 节点混合部署 **DP4/TP16/EP64**（vLLM 数据并行跨 4 节点，每节点 1 个 DP rank，张量并行用满单节点 16 NPU）。
- **镜像**：`quay.io/ascend/vllm-ascend:kimi-k3-a3`（Ubuntu）/ `quay.io/ascend/vllm-ascend:kimi-k3-a3-openeuler`（openEuler）。
- **关键启动参数**：`--tensor-parallel-size 16`、`--data-parallel-size 4`、`--data-parallel-size-local 1`、`--enable-expert-parallel`、`--max-model-len 131027`、`--max-num-batched-tokens 24576`、`--max-num-seqs 16`、`--gpu-memory-utilization 0.9`、`--compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY"}'`、`--additional-config '{"enable_cpu_binding":true,"enable_flashcomm1":true}'`、`--mm-encoder-tp-mode data`、`--limit-mm-per-prompt '{"vision_chunk":40}'`、`--enable-auto-tool-choice`、`--reasoning-parser kimi_k3`、`--tool-call-parser kimi_k3`、`--enable-prefix-caching`。
- **关键环境变量**：`HCCL_IF_IP=$LOCAL_IP`、`GLOO_SOCKET_IFNAME/$TP_SOCKET_IFNAME/$HCCL_SOCKET_IFNAME=$NIC_NAME`、`VLLM_ENGINE_READY_TIMEOUT_S=7200`、`PYTORCH_NPU_ALLOC_CONF=expandable_segments:True`、`OMP_PROC_BIND=false`、`OMP_NUM_THREADS=1`、`TASK_QUEUE_ENABLE=1`、`HCCL_BUFFSIZE=800`、`ASCEND_RT_VISIBLE_DEVICES=0,1,...,15`。
- **多节点启动顺序**：先起 Node 0；Node 1/2/3 设 `NODE0_IP` 为 Node0 的 `LOCAL_IP`，并分别给 `--data-parallel-start-rank` 设 1/2/3。

## vllm serve 启动命令（原文保留，Node 0）
...
> 以下为 `Kimi-K3.md` 原文全文（releases/v0.23.0 分支），作为部署指导权威来源。原文中的 `{tab-set}`/`{tab-item}` 等 mkdocs 标记保留以便对照渲染版。
