> 来源：vllm-ascend docs/source/tutorials/models/DeepSeek-V4-Flash.md（main 分支，抓取于 2026-07-28）

# DeepSeek-V4-Flash 部署知识库

## 关键信息速览（中文提炼）

- 架构类型：MoE
- 推荐 TP：[4, 8]
- DP（data-parallel-size）：[1, 4]
- max-model-len：[133120, 135000, 800000, 1048576]
- gpu-memory-utilization：['0.9']
- enforce-eager：出现（建议 Eager 模式，见原文上下文）
- 是否有量化：是（W8A8, W4A8）
- 相关环境变量：ASCEND_RT_VISIBLE_DEVICES, GLOO_SOCKET_IFNAME, HCCL_BUFFSIZE, HCCL_CONNECT_TIMEOUT, HCCL_EXEC_TIMEOUT, HCCL_IF_IP, HCCL_OP_EXPANSION_MODE, HCCL_SOCKET_IFNAME, IMAGE, LD_PRELOAD, OMP_NUM_THREADS, OMP_PROC_BIND, PYTORCH_NPU_ALLOC_CONF, TASK_QUEUE_ENABLE, TP_SOCKET_IFNAME, VLLM_ASCEND_ENABLE_FLASHCOMM1, VLLM_EXECUTE_MODEL_TIMEOUT_SECONDS, VLLM_RPC_TIMEOUT
