> 来源：vllm-ascend docs/source/tutorials/models/Qwen3.5-27B-Qwen3.6-27B.md（main 分支，抓取于 2026-07-28）

# Qwen3.5-27B-Qwen3.6-27B 部署知识库
...
- max-model-len：[16384, 32768, 133000, 262144]
- gpu-memory-utilization：['0.90', '0.91', '0.95']
- enforce-eager：出现（建议 Eager 模式，见原文上下文）
- 是否有量化：是（W8A8）
- 相关环境变量：ASCEND_RT_VISIBLE_DEVICES, GLOO_SOCKET_IFNAME, HCCL_BUFFSIZE, HCCL_IF_IP, HCCL_OP_EXPANSION_MODE, HCCL_SOCKET_IFNAME, IMAGE, LD_LIBRARY_PATH, LD_PRELOAD, MODEL_PATH, OMP_NUM_THREADS, OMP_PROC_BIND, PYTORCH_NPU_ALLOC_CONF, TASK_QUEUE_ENABLE, TP_SOCKET_IFNAME, VLLM_ASCEND_ENABLE_FLASHCOMM1, VLLM_USE_MODELSCOPE
