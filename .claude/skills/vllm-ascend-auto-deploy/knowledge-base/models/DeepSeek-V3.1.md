> 来源：vllm-ascend docs/source/tutorials/models/DeepSeek-V3.1.md（main 分支，抓取于 2026-07-28）
...
- max-model-len：[16384, 65536]
- gpu-memory-utilization：['0.9', '0.92']
- enforce-eager：出现（建议 Eager 模式，见原文上下文）
- 是否有量化：是（W8A8, W4A8）
- 相关环境变量：ASCEND_BUFFER_POOL, ASCEND_RT_VISIBLE_DEVICES, GLOO_SOCKET_IFNAME, HCCL_BUFFSIZE, HCCL_CONNECT_TIMEOUT, HCCL_EXEC_TIMEOUT, HCCL_IF_IP, HCCL_INTRA_PCIE_ENABLE, HCCL_INTRA_ROCE_ENABLE, HCCL_OP_EXPANSION_MODE, HCCL_SOCKET_IFNAME, IMAGE, LD_LIBRARY_PATH, LD_PRELOAD, OMP_NUM_THREADS, OMP_PROC_BIND, PYTORCH_NPU_ALLOC_CONF, TASK_QUEUE_ENABLE, TP_SOCKET_IFNAME, VLLM_ASCEND_BALANCE_SCHEDULING
