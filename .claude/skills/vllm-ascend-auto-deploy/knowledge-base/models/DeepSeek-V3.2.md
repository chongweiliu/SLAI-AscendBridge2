> 来源：vllm-ascend docs/source/tutorials/models/DeepSeek-V3.2.md（main 分支，抓取于 2026-07-28）
...
- 架构类型：MoE
- 推荐 TP：[8, 16]
- DP（data-parallel-size）：[2]
- max-model-len：[8192, 68000]
- gpu-memory-utilization：['0.82', '0.92', '0.95']
- enforce-eager：出现（建议 Eager 模式，见原文上下文）
- 是否有量化：是（W8A8）
- 相关环境变量：ACL_OP_INIT_MODE, ASCEND_A3_ENABLE, ASCEND_AGGREGATE_ENABLE, ASCEND_RT_VISIBLE_DEVICES, ASCEND_TRANSPORT_PRINT, GLOO_SOCKET_IFNAME, HCCL_BUFFSIZE, HCCL_CONNECT_TIMEOUT, HCCL_IF_IP, HCCL_INTRA_PCIE_ENABLE, HCCL_INTRA_ROCE_ENABLE, HCCL_OP_EXPANSION_MODE, HCCL_SOCKET_IFNAME, IMAGE, OMP_NUM_THREADS, OMP_PROC_BIND, PYTORCH_NPU_ALLOC_CONF, TASK_QUEUE_ENABLE, TP_SOCKET_IFNAME, VLLM_ASCEND_ENABLE_FLASHCOMM1
