> 来源：vllm-ascend docs/source/tutorials/models/Mixtral-8x7B-Instruct-v0.1.md（main 分支，抓取于 2026-07-28）

# Mixtral-8x7B-Instruct-v0.1 部署知识库

## 关键信息速览（中文提炼）

- 架构类型：MoE
- 推荐 TP：[4]
- max-model-len：[512, 4096]
- gpu-memory-utilization：['0.7']
- enforce-eager：出现（建议 Eager 模式，见原文上下文）
- 是否有量化：是（存在量化配置）
- 相关环境变量：HCCL_BUFFSIZE, HCCL_OP_EXPANSION_MODE, IMAGE, OMP_NUM_THREADS, OMP_PROC_BIND, PYTORCH_NPU_ALLOC_CONF, VLLM_ASCEND_BALANCE_SCHEDULING, VLLM_ASCEND_ENABLE_FLASHCOMM1, VLLM_ASCEND_ENABLE_MLAPO, VLLM_USE_V1
