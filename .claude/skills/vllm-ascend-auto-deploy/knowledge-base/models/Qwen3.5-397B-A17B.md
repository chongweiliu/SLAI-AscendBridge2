> 来源：vllm-ascend docs/source/tutorials/models/Qwen3.5-397B-A17B.md（main 分支，抓取于 2026-07-28）

# Qwen3.5-397B-A17B 部署知识库

## 关键信息速览（中文提炼）

- 架构类型：MoE
- 推荐 TP：[2, 8, 16]
- DP（data-parallel-size）：[1, 2, 8, 16]
- max-model-len：[16384, 32768, 133000]
- gpu-memory-utilization：['0.9', '0.90', '0.96']
- enforce-eager：出现（建议 Eager 模式，见原文上下文）
- 是否有量化：是（W8A8）
- 相关环境变量：ASCEND_RT_VISIBLE_DEVICES, GLOO_SOCKET_IFNAME, HCCL_BUFFSIZE, HCCL_IF_IP, HCCL_OP_EXPANSION_MODE, HCCL_SOCKET_IFNAME, IMAGE, IP_ADDRESS, LD_LIBRARY_PATH, LD_PRELOAD, MASTER_IP_ADDRESS, NETWORK_CARD_NAME, OMP_NUM_THREADS, OMP_PROC_BIND, PYTORCH_NPU_ALLOC_CONF, TASK_QUEUE_ENABLE, TP_SOCKET_IFNAME, VLLM_ENGINE_READY_TIMEOUT_S, VLLM_MOONCAKE_ABORT_REQUEST_TIMEOUT, VLLM_TORCH_PROFILER_WITH_STACK
