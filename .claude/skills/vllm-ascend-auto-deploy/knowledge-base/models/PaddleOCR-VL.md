> 来源：vllm-ascend docs/source/tutorials/models/PaddleOCR-VL.md（main 分支，抓取于 2026-07-28）
...
- 架构类型：VLM
- 推荐 TP：见原文（未显式标注或视节点规模而定）
- enforce-eager：出现（建议 Eager 模式，见原文上下文）
- 是否有量化：否（以原精度 BF16/FP16 部署为主）
- 相关环境变量：ASCEND_RT_VISIBLE_DEVICES, CPU_AFFINITY_CONF, IMAGE, MODEL_PATH, PYTORCH_NPU_ALLOC_CONF, TASK_QUEUE_ENABLE, VLLM_USE_MODELSCOPE
