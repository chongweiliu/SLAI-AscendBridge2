> 来源：vllm-ascend docs/source/tutorials/models/GLM5.md（main 分支，抓取于 2026-07-28）
...
- 架构类型：MoE
- 推荐 TP：[8, 16]
- DP（data-parallel-size）：[1, 2]
- max-model-len：[8192, 32768, 40960, 131072, 200000]
- gpu-memory-utilization：['0.92', '0.95']
- enforce-eager：出现（建议 Eager 模式，见原文上下文）
- 是否有量化：是（W8A8, W4A8）
- 相关环境变量：ACL_OP_INIT_MODE, ASCEND_A3_ENABLE, ASCEND_AGGREGATE_ENABLE, ASCEND_RT_VISIBLE_DEVICES, ASCEND_TRANSPORT_PRINT, GLOO_SOCKET_IFNAME, HCCL_BUFFSIZE, HCCL_IF_IP, HCCL_OP_EXPANSION_MODE, HCCL_SOCKET_IFNAME, IMAGE, LD_LIBRARY_PATH, OMP_NUM_THREADS, OMP_PROC_BIND, PYTORCH_NPU_ALLOC_CONF, TASK_QUEUE_ENABLE, TP_SOCKET_IFNAME, VLLM_ASCEND_BALANCE_SCHEDULING, VLLM_ASCEND_ENABLE_FLASHCOMM1, VLLM_ASCEND_ENABLE_FUSED_MC2
