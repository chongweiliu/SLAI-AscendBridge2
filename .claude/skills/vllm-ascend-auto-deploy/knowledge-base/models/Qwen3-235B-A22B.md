> 来源：vllm-ascend docs/source/tutorials/models/Qwen3-235B-A22B.md（main 分支，抓取于 2026-07-28）

# Qwen3-235B-A22B 部署知识库
...
- max-model-len：[32768, 40960, 131072, 135000]
- gpu-memory-utilization：['0.85', '0.9', '0.95']
- enforce-eager：出现（建议 Eager 模式，见原文上下文）
- 是否有量化：是（W8A8）
- 相关环境变量：ASCEND_RT_VISIBLE_DEVICES, GLOO_SOCKET_IFNAME, HCCL_BUFFSIZE, HCCL_IF_IP, HCCL_OP_EXPANSION_MODE, HCCL_SOCKET_IFNAME, IMAGE, LD_LIBRARY_PATH, LD_PRELOAD, OMP_NUM_THREADS, OMP_PROC_BIND, PYTORCH_NPU_ALLOC_CONF, TASK_QUEUE_ENABLE, TP_SOCKET_IFNAME, VLLM_TORCH_PROFILER_WITH_STACK
