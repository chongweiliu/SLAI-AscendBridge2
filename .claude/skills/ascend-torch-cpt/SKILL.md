...
- **开工先验权重可训性**（safetensors/bin/pt 可训；GGUF/ONNX/空目录不可训→同源 HF repo 或标准社区 checkpoint 替代, #153）。**CPT 前必扫权重 NaN**（#101）：safetensors 尺寸正确 ≠ 内容正确（比特级损坏致 lm_head 80 NaN）；smoke 前用 `load_file` 全量 `torch.isnan` 扫描（2B~30s, 7B~3min），出 NaN 先 CPU 前向确认再重下损坏分片。
- 非 PyTorch 原生权重（Keras `.pkl`/`.h5`）：pickle 纯 numpy 元组可直接解包（无需 TF），逐层复刻 PyTorch 架构后做**形状严格校验+同形交换消融+语义 sanity**三重验证（#84–#86）。
- **ViT/Science 模型需额外依赖**：`pip install timm --no-deps`（避免拉入 CUDA torch 覆盖 torch_npu, #114）；GeoTIFF 数据需 `pip install rasterio`；tar.gz 中批量读 GeoTIFF 极慢，先预提取到磁盘（#111）。**diffusers 生成式**也需 `pip install diffusers accelerate --no-deps`，pipeline 加载失败时直接加载组件（#117），fp16 权重需 `variant="fp16"`。
- 数据集只有 train 分割时用 seed 重建 held-out（references/eval-metrics.md）。

### 阶段 3 · 语料格式转换与打包（按范式分支）
- **A. 文本 LM**：读语料（jsonl/json/parquet/csv）→ 判定格式（chat `{"messages"}` 用 `apply_chat_template`；`{"text"}` 直接 tokenize；其它取可读字段）→ 打包 `seq_len` 定长块（不足步数则循环重采样，记录 epoch 数）→ `scripts/prepare_data.py.tmpl`，**务必打印**总样本/子集样本/总 token/块数/epoch 估计。大语料设 `CAP_TOKENS` 避免 tokenize 浪费（#105）；**Llama/SP 模型禁用 AutoTokenizer**（空格→unk 33%，显式 LlamaTokenizerFast 对拍 sp.encode，#143）。数据边界（去重/分块/混合/packing vs padding）见 references/data-prep.md。**caption/text 字段可能是 list**（flickr 一图多 caption）→ tokenize 全空 → 打包 while 死循环：抽取时 join + 空块保护 + py-spy 定位（#128）。**多卡必设 `WORLD_SIZE=N`**（否则 need 算少→内循环重复采样）。
...
```
经验阈值：权重占单卡 ≤~40% → 单卡；0.5–3B 常单卡/DDP；≥30B → FSDP2/模型并行；步数 <~150 不上图模式（torchair 首图编译 ~15min 摊销不了）；互联慢时大模型勿用 FSDP2（通信-bound，实测 97% 时间在通信）。**torch 2.10 fully_shard 无 device_id 参数**——用 `init_device_mesh("npu",(w,))` 传 mesh=；**超大模型 per-rank 加载偏斜撞 HCCL 120s 默认连接超时**——必设 `HCCL_CONNECT_TIMEOUT=1800` + 加载后 `dist.barrier()`（#137）。**FSDP2 加载唯一正确序列：CPU 上 build+load+fully_shard，之后永不显式 to(dev)**（FSDP2 在 pre_forward 自行流式搬 shard；显式 to 破坏内部状态致 ~5.5GB 假 OOM, #145）；**开工先核对 `npu-smi` die 数与基线——die 掉线后任意 rank 数 HCCL 建链全挂，直接降级单卡**（#149）。

### 阶段 5 · 超参自动择优（references/hyperparam-selection.md）
- **precision**：fp32 主权重 + bf16 autocast（**不要**纯 bf16 前向 #4）；**lr**：CPT 基线 1e-5（全局 batch 大按 sqrt 上调；短训练保守值；照抄基座从零训练的调度会发散 #82）；**warmup** ~10% 步数 cosine 到 0；**optimizer**：`NpuFusedAdamW` + betas(0.9,0.95) wd=0.01 eps=1e-8 clip=1.0——**例外**：分阶段重建优化器（freeze/unfreeze 切换）用 plain AdamW（#87 跨阶段 saved-tensor 崩溃），小模型(<100M)融合无收益；**Adafactor 与 FSDP2 DTensor 不兼容**（in-place pow_）——超大模型装不下 AdamW 时改"冻结下半层+AdamW"而非 Adafactor（#138）；**单卡 bf16 7B 底座装 AdamW states 不下时用 Adafactor(beta1=None)**——beta1≠None 会建全量 exp_avg，factorized 名存实亡（#147）；**batch_size** 按"激活显存预算"估上限+2 步 smoke 验不 OOM（优先开梯度检查点而非降 bs，`use_reentrant=False`）；**attention**：full_attention 走 SDPA（→NPU fusion 自动路由）。

### 阶段 6 · 生成训练脚本并 smoke
<<<<<<< HEAD
- 按范式选模板：文本 `cpt_train.py.tmpl`（单卡+DDP 自动检测）/`cpt_fsdp.py.tmpl`/`cpt_mp.py.tmpl`；**Encoder MLM** `cpt_mlm.py.tmpl`（单卡+DDP）；**Seq2Seq** `cpt_seq2seq.py.tmpl`（单卡+DDP）；扩散 `cpt_diffusion.py.tmpl`；音频 `cpt_audio_llm.py.tmpl`；语音 codec-token LM `cpt_speech_codec.py.tmpl`（TTS talker，配 references/speech-codec-lm-cpt.md）。模板已支持断点续训（`RESUME=1` 默认关）与梯度累积（references/resume.md）。**单卡 set_device(0) 不是 VISIBLE_DEVICES 值**（#106）；MLM 模型 pad_token fallback eos→unk→sep（#108）。
- 模板通用化：文本 `AutoModelForCausalLM.from_pretrained(path, trust_remote_code=True, dtype=torch.float32)`（transformers ≥5 统一 `dtype=`，#142）`；多模态走 remap；**VLM** 用 `*ForConditionalGeneration` 加载（#115）；**ViT/Science 自定义代码模型**用 `sys.path.insert(0, MODEL_DIR)` + 直接 import `*.py`（#112），checkpoint 的 `num_frames`/`img_size` 必须与 config 一致（#110）。**diffusers 版本不匹配**时用 shape-based key remap（#119，通用解法，按 tensor shape 自动匹配 825+ keys）；**SDXL UNet** forward 需 `added_cond_kwargs`（#118）；**大型 text encoder**（T5 11.4GB）可跳过用 zero embeddings（#120）。**>3B 模型 fp32+NpuFusedAdamW 单卡 OOM** → 改 bf16 权重 + `torch.optim.AdamW`（#116）。**非标准 HF 结构（CNN 前端语音/DETR/自定义包模型）遇数字固定的 EZ1001 broadcast 直接换 plain AdamW**（fused kernel 对非常规参数广播崩, #150）。**大模型 H2D 假 OOM（allocated≪卡容量）先设 `torch.npu.config.allow_internal_format=False`**（NZ 转换峰值 ~10×, #144）；**checkpoint 键重排/permute 后参数必须 `.contiguous()`**（FSDP2 拒绝非连续, #146）。**NPU embedding 所有 index tensor（input_ids/bbox/position_ids）必须 `torch.long`**（#121）；LayoutLMv3 tokenizer 用 `backend_tokenizer.encode_batch` 绕过 bbox（#122）。**NPU CE 不做标签越界检查**——标签≥头维度时输出假 loss 0.0，先查 `max(label)<n_cls`（#132）；**旧命名 config**（GOT "GOT"→"got_ocr2"）patch model_type+显式新类（#135）；**原生格式 TTS**（CosyVoice3 llm.pt=Qwen2 骨架）键前缀 remap 后按文本 LM 训（#134）；**DepthPro 必须 processor 1536 且禁 NpuFusedAdamW**（#131）；**Qwen3-ASR 类**须官方 -hf repo+波形整秒补零+input_features_mask+prompt-only 掩码（#129）；原生 release 可能整块缺 audio projector、audio 占位手动构造（秒×13）、**变长 audio batch 必崩须统一固定时长**（#151）。
- **smoke**：2 步确认前向+反向+优化器 step 全通过、loss 合理再上正式；扩散先对 backbone 与 VAE 分别单组件前向 smoke（抓 NPU 算子问题 #50）。**smoke 的 s/step 不可外推正式用时**（首 import ~90s + NpuFusedAdamW 首步状态初始化 ~10×虚高，取稳态第 2 步, #105）。**连续调优后显存纹丝不动 = 配置分支未生效**（重构硬编码覆盖），先打 `param_bytes/grad_bytes/dtype 分布` 探针与配置对账再调优（#148）。
- 踩坑先 grep references/pitfalls.md（模板已规避多数）。**大词表(>100K vocab) CE OOM** 先降 bs+梯度累积保持有效 batch，不降 seq_len（#103）；bs 已为 1 时 bf16 直接算 CE、跳过 logits.float() upcast（#154）。**FSDP2 取 transformer 层勿用 getattr 默认值**（急切求值致 AttributeError, #102，模板已修复）。**梯度累积 zero_grad 必须在累积窗口开头**，放 per-micro-batch 循环内会静默丢梯度（#141，模板已修复）。**自定义双头模型照抄官方 finetune 传参会双重错位**（ForCausalLMLoss 内部 shift，t→t+2；全长 inputs+own-position labels + 多头恒等配对）。
=======
...
- 踩坑先 grep references/pitfalls.md（模板已规避多数）。**大词表(>100K vocab) CE OOM** 先降 bs+梯度累积保持有效 batch，不降 seq_len（#103）。**FSDP2 取 transformer 层勿用 getattr 默认值**（急切求值致 AttributeError, #102，模板已修复）。**梯度累积 zero_grad 必须在累积窗口开头**，放 per-micro-batch 循环内会静默丢梯度（#141，模板已修复）。**自定义双头模型照抄官方 finetune 传参会双重错位**（ForCausalLMLoss 内部 shift，t→t+2；全长 inputs+own-position labels + 多头恒等配对）。
>>>>>>> origin/main
...
**通用**：base vs CPT ①严格同条件（同种子一切随机源+确定性自检）②协议锚定（base 关键指标与论文/官方数字同量级才算协议对）③持平可能是正确结论（短程 CPT 域内收敛基座预期 ±1%）；**过拟合检查**：held-out 须独立 split；train loss 趋 0 + held-out 不改善=红旗；**强 backbone 小数据全参微调的遗忘判定：任务头指标涨+通用/held-out 指标跌=配方错误非 bug**（评估须含 head 无关指标, #152）；**回归/检测/分类头模型走 MLM 范式时 base 对照无意义**（MLM 头随机初始化，#133），如实记录口径以训练收敛佐证；**数据量是泛化关键杠杆**（非步数）。多卡训练后别立刻单卡评估（显存异步回收 #32）；评估载入前 `torch.npu.empty_cache()`。
...
| **基因组 SSM 混合** (EVO2/StripedHyena2, 原生 .pt+vortex 源码) | next-token CE（字节级, eod 分隔） | 源码 sdist 提取+NPU patch（#144-146） | 字节 PPL/acc (held-out) |
| **语音/OCR/时序/分割专用** (ASR/TTS/wavlm/sam3/GLM-OCR/chronos-2) | 转写 CE / codec CE / 自蒸馏 / oracle-query focal+dice / OCR CE / quantile | HF 原生类或隔离 pylibs 包（#155） | WER·CER·IoU·masked-MSE·quantile MSE；**须含 head 无关指标防遗忘误判**（#152） |
...
工具 `scripts/timing_table.py.tmpl`（读 `outputs/timing.json`），触发点=核心原则 9 的 T1–T6。要求：整体预估（T1 后写入 `overall_estimate_s`）；进入即 `--doing`、完成即 `--set actual`；长跑阶段按速率外推 ETA 每 1–2min 刷新；**收尾汇总（T6）最终答复与 README 各带一份完整表**。格式（列：阶段/预计/实际/进度/状态/说明；doing 的 note 含 "N/M" 自动算阶段%，`--pct` 可显式指定；合计行=整体%+剩余 ETA+预计完成时刻）：

```
| 阶段 | 预计 | 实际 | 进度 | 状态 | 说明 |
|---|---|---|---|---|---|
| 0 意图确认与路径核对 | 1.0min | 0.5min | 100% | ✅done | 路径已核对 |
| 7 正式训练+曲线 | 6.0min | 2.9min | 50% | ⏳doing | 50/100步 loss2.3 |
| **合计** | **~22min** | **10.5min** | **68%** | **进行中** | 剩余 ~7min 预计完成 14:35 |
```

长跑期间可只追加 `⏳doing` 行（带已耗/百分比/ETA）；合计行始终显示"已实际/整体预估/整体% /剩余/预计完成时刻"。
