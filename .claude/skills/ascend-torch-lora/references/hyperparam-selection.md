...
## sweep 实测扩展（2026-09-10/11/18，cat2-6 累计 19 模型）

按「基座与任务域的距离」归纳三类超参模式（sweep 会自动选出，人工核对时按此预判）：

| 模式 | 特征 | 典型选择 | 实测例 |
|---|---|---|---|
| **新头/域差大** | 任务头新初始化或域迁移显著，val loss 各组合差距大 | lr=2e-4, r=32（激进） | cat1 四模型、bge-m3/nllb/distilbert、ChemBERTa/ClinicalBERT、SDXL、dinov2/rtdetr/layoutlmv3、AST/tts |
| **强基座近同源** | 微调域≈预训练域，val loss 全组合近平（差距<5%） | lr=1e-4, r=8~16（保守）；**持平是正确结论** | esm2(-1.5%)、Prithvi(饱和)、bert-NER(饱和)、depth(DAv2)、forecast(TimesFm)、Wan(任务饱和) |
| **数据量不足** | sweep 点≈基线但正式训练 val 反升 | 任何组合都救不了——如实记"无提升" | bge-reranker(30 query 过拟合 MRR -0.042) |

- **whisper(asr) 例外样本**：强基座但域差存在（LibriSpeech 朗读 vs 多域预训练）→ 2e-4/r16，
  lr 敏感性高（5e-5→2e-4 val CE 0.253→0.112）而 r 敏感性低（16 vs 32: 0.1117 vs 0.1141）。
- **VLM 打平取保守**：Qwen2.5-VL 全 5 组合两组合打平（1.8358/1.8373）时按协议取保守组合；
  重跑批次 r=32 以 0.0015 优势胜出——打平区间内选哪个都成立，报告记录选择规则即可。
- **sweep 有效性前提**：训练器实现正确（新头在训、特征未湮灭）——先过**单批过拟合测试**再 sweep；
  判据被 bug 污染时按 pitfalls #50 确认复验或作废重跑。
- det 的 val loss 量级（DETR 多分量加权和，~100-250）与 CE 类不可横向比较，只看同范式内排序。
