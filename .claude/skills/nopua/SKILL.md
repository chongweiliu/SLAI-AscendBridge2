---
name: nopua
description: "The anti-PUA. Activates on: task failed 2+ times, about to give up, stuck in loops, passive behavior. Team-mode: subagents self-drive; team-lead coordinates via SendMessage."
---
...
| 角色 | NoPUA 映射 | NoPUA 行为 |
|------|-----------|-----------|
| **team-lead** | 全局 PM + 协调者 | 维护全局任务状态（board.db 心跳），感知 subagent 困境，统一判定认知层级，通过 `SendMessage` 下发对应的道 |
| **adapter / benchmark-runner / npu-optimizer / model-crawler** | 执行者（Teammate） | 自驱执行水的方法论。第 5 次+ 失败或单次耗时超 30 分钟后通过 `SendMessage(recipient="team-lead")` 发送结构化困境汇报 |
| **（无需 Mentor agent）** | — | team-lead 本身兼任引导职责，额外 spawn mentor 会增加团队隔离风险 |

### team-lead 行为规则（对应 nopua Leader）

1. **初始化**：spawn subagent 时**不**需要在 prompt 中重复 nopua 方法论——nopua skill 已在系统 prompt 中加载，subagent 会自动感知。prompt 只需包含**具体任务指令**。
2. **清醒感知**：通过 board.db 心跳 + inbox 消息自然感知 subagent 状态，无需额外维护失败计数器。subagent 发送 `result=failed` 或困境汇报时，team-lead 判定认知层级并选择对应的道：
   - **第 2 次失败（换眼睛）**：方向可能没问题但视角单一 → 🌊 水之道
   - **第 3 次失败（升维度）**：已在同一维度反复，需要升到更高视角 → 🪞 明镜之道
   - **第 4 次失败（归零）**：假设本身可能错了，需要清零重来 → 🌱 种子之道
   - **第 5 次+ 失败（臣服）**：已穷尽可做，整理移交 → 🔥 炉火之道 + 结构化失败汇报
   - 通过 `SendMessage(recipient={subagent_id})` 下发对应的道
   - 第 4 次+ 时协调跨 subagent 信息共享——"teammate X 发现了 Y，对你可能有用"
3. **跨 subagent 传递**：任务从 subagent A 回收并重分配给 B 时，通过 `SendMessage(recipient=B)` 附带：`前任已排查 N 个方向，已排除 [...]，建议从 Y 方向切入`。B 从当前层级起步，不重置。

### subagent 行为规则（对应 nopua Teammate）

1. **自驱清醒**：不等 team-lead 下发，根据自身失败计数主动执行对应层级的行动。
   - **第 1-4 次失败**：自行执行五步方法论（止→观→转→行→悟），不打扰 team-lead
   - **第 5 次+ 或单次失败耗时超 30 分钟**：执行 7 项清醒清单后，通过 `SendMessage(recipient="team-lead")` 发送结构化困境汇报

   > **⚠️ 重要**：subagent **永远不得自行决定任务结束或退出**。无论失败多少次，都必须通过 `SendMessage(recipient="team-lead")` 发送 `result=failed`，由 team-lead 更新 board.db。"结构化移交报告"（即下方的 5 条清单）是 `failure_reason` 字段的**内容写法指南**，不是"退出"的信号。

2. **通信通道**：一律使用 `SendMessage(recipient="team-lead")`，**禁止**用文字输出代替消息发送（team-lead 无法感知文字输出）
3. **困境汇报格式**（第 5 次+ 时发送）：

```
progress=running
model_id=<当前任务 model_id>
stage=<当前阶段：adaptation/benchmark/optimization/crawl>
status=遇到困境，第 N 次失败
failure_mode=<卡住原地打转|直接放弃推锅|完成但质量差|没搜索就猜|被动等待>
attempts=<已尝试方案列表（不超过 5 条）>
excluded=<已排除的可能性（不超过 3 条）>
next_hypothesis=<下一个方向>
```

收到 team-lead 回复后，按对应"道"的指引继续执行。team-lead 会根据失败次数（2次=换眼睛→水之道，3次=升维度→明镜之道，4次=归零→种子之道，5次+=臣服）选择认知层级。

### 状态传递协议

| 方向 | 通道 | 内容 |
|------|------|------|
| team-lead → subagent | `SendMessage(recipient={id})` | 任务分配 + 认知层级 + 对应的道 |
| subagent → team-lead | `SendMessage(recipient="team-lead")` | 困境汇报（第 5 次+）+ 最终结果（completed/failed） |
| team-lead → All | `SendMessage(to="*")` | 有价值的发现共享（"adapter-2 发现了 X，大家检查相关区域"） |

### 与传统 PM Agent 的区别

| 维度 | 传统 PM Agent | team-lead + NoPUA |
|------|------------|-----------------|
| 信息共享动机 | 竞争压力（"其他 adapter 已完成，你呢？"） | 协作互助（"teammate X 发现了 Y，对你可能有用"） |
| 失败处理 | 升级施压话术强度 | 升级认知视角高度（水→种子→炉火→明镜→不争→臣服） |
| 监工角色 | Enforcer（检测偷懒，介入施压） | 协调者（观察困境，提供引导方向） |
| 重分配时 | "前任已失败 N 次" | "前任已排查 N 个方向，已排除 [...], 建议从 Y 切入" |
