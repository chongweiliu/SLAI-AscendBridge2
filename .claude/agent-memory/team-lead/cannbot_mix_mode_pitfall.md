...
description: cannbot 融合 MIX 模式 (__mix__(1,2)) kernel 的 AIC 驱动陷阱与回退策略（稀疏注意力算子 实战教训）
metadata:
  type: project
---

## 核心教训（2026-07-13 稀疏注意力算子 实战）
...
- **中间矩阵显存**：非融合会在 GM 存中间结果。稀疏注意力算子 的 qk_score `[num_q_heads, total_q_len, compressed_k_len]` ~10GB，Ascend910 64GB HBM 可接受。设计时算好显存预算。
