...
//   + cannbot 自带 4 角色（运行 scripts/sync_cannbot.sh 后从 .cache/cannbot 显式读取），不引入新 agent。
...
if (gap.has_gap && ascendOps.length > 0) {
  ...
  function archPrompt(op) {
    return [
      commonPreamble(ctx),
      '',
      '先运行 $PROJECT_ROOT/scripts/sync_cannbot.sh --print-path 获取 CANNBOT_ROOT，并读取 $CANNBOT_ROOT/plugins-official/ops-direct-invoke/agents/ascendc-kernel-architect.md 与其中点名的 Skills。禁止执行上游 init.sh、根级 AGENTS.md/CLAUDE.md 或 hooks。',
  ...
  function walkPrompt(_prev, op) {
    return [
      commonPreamble(ctx),
      '',
      '先运行 $PROJECT_ROOT/scripts/sync_cannbot.sh --print-path 获取 CANNBOT_ROOT，并读取 $CANNBOT_ROOT/plugins-official/ops-direct-invoke/agents/ascendc-kernel-design-reviewer.md 与其中点名的 Skills。禁止执行上游 init.sh、根级 AGENTS.md/CLAUDE.md 或 hooks。',
  ...
  function devPrompt(_prev, op) {
    return [
      commonPreamble(ctx),
      '',
      '先运行 $PROJECT_ROOT/scripts/sync_cannbot.sh --print-path 获取 CANNBOT_ROOT，并读取 $CANNBOT_ROOT/plugins-official/ops-direct-invoke/agents/ascendc-kernel-developer.md 与其中点名的 Skills。禁止执行上游 init.sh、根级 AGENTS.md/CLAUDE.md 或 hooks。',
  ...
  function reviewPrompt(_prev, op) {
    return [
      commonPreamble(ctx),
      '',
      '先运行 $PROJECT_ROOT/scripts/sync_cannbot.sh --print-path 获取 CANNBOT_ROOT，并读取 $CANNBOT_ROOT/plugins-official/ops-direct-invoke/agents/ascendc-kernel-reviewer.md 与其中点名的 Skills。禁止执行上游 init.sh、根级 AGENTS.md/CLAUDE.md 或 hooks。',
  ...
  const cannbotResults = await pipeline(
    ascendOps,
    function (op) {
      return agent(archPrompt(op), { agentType: 'AGENTS', schema: ARCH_SCHEMA, phase: 'CannbotDev', label: 'arch:' + op.op_name })
    },
    function (_prev, op) {
      return agent(walkPrompt(_prev, op), { agentType: 'AGENTS', schema: WALK_SCHEMA, phase: 'CannbotDev', label: 'walk:' + op.op_name })
    },
    function (_prev, op) {
      return agent(devPrompt(_prev, op), { agentType: 'AGENTS', schema: DEV_SCHEMA, phase: 'CannbotDev', label: 'dev:' + op.op_name })
    },
    function (_prev, op) {
      return agent(reviewPrompt(_prev, op), { agentType: 'AGENTS', schema: CANNBOT_REVIEW_SCHEMA, phase: 'CannbotDev', label: 'rev:' + op.op_name })
    }
  )
